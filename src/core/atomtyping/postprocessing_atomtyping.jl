...
function gaff_postprocessing_all_conjugated_systems!(mol::AbstractMolecule)
    ...
    cat_chem_cycles_array = isempty(mol.properties["chem_cycle_array"]) ? Vector{Int}() : reduce(vcat, mol.properties["chem_cycle_array"])
    ...
    for conjAtm in eachrow(group1_conjugated_atoms)
        ...
        if nrow(neighbors_in_conjugated_atoms) == 1 && !in(seen_atoms_vec).(conjAtm.number[1])
            ...
            println(all_paths_from_curr_conjAtm)
        end
    end
end
...
function conjAtm_path_builder!(group1_conjugated_atoms::DataFrame, path_vec::Vector{Int}, mol::AbstractMolecule, depth::Int)
    ...
    println(bond_in_conjugated_atoms)
    ...
    println(next_neighbors_vec, " isempty:  ", isempty(next_neighbors_vec))
